...
namespace PrjModule25_Parser.Service.Helpers
{
    ...
    {
        public static async Task AddProductsFromSellerPageToDb(ShopData shop, IDocument sellerPage, ApplicationDb dbContext, IHubContext<ApiHub, IApiClient> shopHub)
        {
            ...
            for (var i = 1; i <= pageCount; i++)
            {
                ...
                var linksSublistPage = page.QuerySelectorAll("*[data-qaid='product_link']").ToList().Cast<IHtmlAnchorElement>()
                ...
                {
                    await shopHub.Clients.All.ReceiveMessage($"Currently parsing shop with name \"{shop.Name}\" \n Already done \"{currentParsingPercent}%\" pages");
                }
